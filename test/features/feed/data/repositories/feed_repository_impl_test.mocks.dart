// Mocks generated by Mockito 5.4.6 from annotations
// in best_flutter/test/features/feed/data/repositories/feed_repository_impl_test.dart.
// Do not manually edit this file.

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'dart:async' as _i4;

import 'package:best_flutter/features/feed/data/datasources/feed_remote_datasource.dart'
    as _i3;
import 'package:best_flutter/features/feed/data/models/post_model.dart' as _i2;
import 'package:mockito/mockito.dart' as _i1;

// ignore_for_file: type=lint
// ignore_for_file: avoid_redundant_argument_values
// ignore_for_file: avoid_setters_without_getters
// ignore_for_file: comment_references
// ignore_for_file: deprecated_member_use
// ignore_for_file: deprecated_member_use_from_same_package
// ignore_for_file: implementation_imports
// ignore_for_file: invalid_use_of_visible_for_testing_member
// ignore_for_file: must_be_immutable
// ignore_for_file: prefer_const_constructors
// ignore_for_file: unnecessary_parenthesis
// ignore_for_file: camel_case_types
// ignore_for_file: subtype_of_sealed_class
// ignore_for_file: invalid_use_of_internal_member

class _FakePostModel_0 extends _i1.SmartFake implements _i2.PostModel {
  _FakePostModel_0(Object parent, Invocation parentInvocation)
    : super(parent, parentInvocation);
}

/// A class which mocks [FeedRemoteDataSource].
///
/// See the documentation for Mockito's code generation for more information.
class MockFeedRemoteDataSource extends _i1.Mock
    implements _i3.FeedRemoteDataSource {
  MockFeedRemoteDataSource() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i4.Future<List<_i2.PostModel>> getPosts({
    required int? page,
    required int? limit,
  }) =>
      (super.noSuchMethod(
            Invocation.method(#getPosts, [], {#page: page, #limit: limit}),
            returnValue: _i4.Future<List<_i2.PostModel>>.value(
              <_i2.PostModel>[],
            ),
          )
          as _i4.Future<List<_i2.PostModel>>);

  @override
  _i4.Future<_i2.PostModel> createPost({
    required String? title,
    required String? body,
    required int? userId,
  }) =>
      (super.noSuchMethod(
            Invocation.method(#createPost, [], {
              #title: title,
              #body: body,
              #userId: userId,
            }),
            returnValue: _i4.Future<_i2.PostModel>.value(
              _FakePostModel_0(
                this,
                Invocation.method(#createPost, [], {
                  #title: title,
                  #body: body,
                  #userId: userId,
                }),
              ),
            ),
          )
          as _i4.Future<_i2.PostModel>);

  @override
  _i4.Future<_i2.PostModel> getPost(int? id) =>
      (super.noSuchMethod(
            Invocation.method(#getPost, [id]),
            returnValue: _i4.Future<_i2.PostModel>.value(
              _FakePostModel_0(this, Invocation.method(#getPost, [id])),
            ),
          )
          as _i4.Future<_i2.PostModel>);

  @override
  _i4.Future<_i2.PostModel> updatePostReactions(int? id, int? likes) =>
      (super.noSuchMethod(
            Invocation.method(#updatePostReactions, [id, likes]),
            returnValue: _i4.Future<_i2.PostModel>.value(
              _FakePostModel_0(
                this,
                Invocation.method(#updatePostReactions, [id, likes]),
              ),
            ),
          )
          as _i4.Future<_i2.PostModel>);
}
